...
// Fill buffer with process information for all active processes
...
getptable(int nproc, char *buffer)
{
  ...
  for(p = proc; p < &proc[NPROC] && count < nproc; p++){
    ...
    if(p->state != UNUSED){
      ...
      pinfo.state = p->state;
      pinfo.sz = p->sz;
      memmove(pinfo.name, p->name, 16);

      // Copy process information to buffer
      if(copyout(myproc()->pagetable, (uint64)(buffer + count * sizeof(struct proc_info)),
